...
static void create_networks() {
	...
	config_network(0, "eduroam", "WPA-EAP", "PEAP", "CCMP", "ingi1@wifi.uclouvain.be", "OLIelmdrad99", "/etc/wpa_supplicant/chain-radius.pem", "peaplabel=0", "auth=MSCHAPV2", NULL);

	config_network(1, "UCLouvain", "WPA-EAP", "TTLS", NULL, "ingi1@wifi.uclouvain.be", "OLIelmdrad99", "/etc/wpa_supplicant/chain-radius.pem", NULL, "auth=PAP", NULL);
	
	config_network(2, "visiteurs.UCLouvain", "WPA-EAP", "TTLS", NULL, "ingi1@wifi.uclouvain.be", "OLIelmdrad99", "/etc/wpa_supplicant/chain-radius.pem", NULL, "auth=PAP", NULL);

	config_network(3, "UCLouvain-prive", "NONE", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);

	config_network(4, "student.UCLouvain", "WPA-EAP", "TTLS", NULL, "ingi1@wifi.uclouvain.be", "OLIelmdrad99", "/etc/wpa_supplicant/chain-radius.pem", NULL, "auth=PAP", NULL);

	config_network(5, "bbox2-f4ba", "WPA-PSK", NULL, NULL, NULL, NULL, NULL, NULL, NULL, "32A273F5A2");
}
...
void *connection_loop(void * p_data) {
	...
	while(1) {
		...
		log_event(LOG_START_CONNECTION_LOOP);
		...
		debug_print("Connect to maison\n");
		log_event(LOG_START_CONNECTION_LOOP, NULL);
		execute_action(ACTION_CONNECT_MAISON);
		services_loop();
		sleep(DELAY);
		debug_print("Disconnect from maison\n");
		execute_action(ACTION_DISCONNECT);
		log_event(LOG_STOP_CONNECTION_LOOP, NULL);
		sleep(DELAY);
