...
\paragraph*{Storage Engine} During the development phase, we used \emph{MySql 5.1}. The storage engine of this version which is \emph{MyISAM}. Since the version \texttt{5.5} of \emph{MySql}, the \emph{InnoDB} engine become the one by default. The documentation of \emph{Django} recommends to use it and we will do the same for the people who desire to deploy our system. Several websites compare the two of them and benchmarks are freely available on the web. 
...
\subsection{Analysis}
It is the second part of our system. It defines the modules that use the gathered data to generate new information. For now, we have define two modules: \texttt{computation} and \texttt{observation}. The main purpose of this separation is the keep together the similar methods of analyse. More information about this are available in the following chapters.

\paragraph*{Computation} In this module, we can find all the methods that are used to aggregate together the data. For example, the \texttt{getUsersByDot11Protocol()} is the one used to generate the graph displaying the repartition of the users among the \emph{802.11} standards. All the methods of this set have a similar role and each of them are related to a particular type of information.

\paragraph*{Observation} Here, we have regrouped methods that analyse a set of data and try to detect certain pattern or anomalies to issue some alert. A really simple example would be the \texttt{isDhcpActive()} that look in the database to determine if some recent \emph{DHCP} activity have been detected.
...
The key is 2048 bits long and each entity has its own pair of keys (i.e. the server and each probe). The server will known the authorized probes by having their private keys and will use them to encrypt the \emph{AES} key (see below).
...
To be correct, the exchanges between the probes and the server requires a protocol well defined and respected by the two parts. As already said, the server knows the public key of every probe in advance and can use them to encrypt the \emph{AES} key. The next step is the transmission of that key and a \emph{Initialization Vector}. When the two parts own the \emph{symmetric} key, the exchange can be encrypted with light computation. The data are sent to the server and it can parse them to store the information on the database.
% Put diagramme sequence
\paragraph*{OpenSSL}
On the \emph{OpenWRT} router, the encryption are realized by the \emph{OpenSSL} library. These methods offers abstraction to use \emph{RSA} and \emph{AES}.
\paragraph*{PyCrypto} As the server is written in \emph{Python}, we used the \emph{PyCrypto} module that offers a wrapper for the cryptographic functions required by our protocol.
