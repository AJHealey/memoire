...
As explained earlier, besides the gathering of data through \texttt{SNMP} protocol or controller's logs parsing, we also provide an active data gathering process. To do that, we have implemented a \texttt{C} program that runs on a \texttt{OpenWrt} router.

\subsection{Linksys Router}
The routers we use for the active monitoring procedure are 150Mbps Wireless N \texttt{TP-LINK TL-WR741ND} routers. Those routers, based on \texttt{N} technology, offers a high speed WiFi performance and are compatible with \texttt{IEEE 802.11b/g/n}. They offer several interesting wireless features\cite{tplink}:
